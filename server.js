...
socketIo.on("connection", (socket) => {
  ...
  socket.on("deleteMessage", (idMessage) => {
    ...
    if (messageToDelete != -1)
      messages[messages.findIndex((x) => x.id === idMessage)].status =
        "deleted";
    ...
    socketIo.emit(
      messages[messages.findIndex((x) => x.id === idMessage)].sender,
      messages
    );
    socketIo.emit(
      messages[messages.findIndex((x) => x.id === idMessage)].receiver,
      messages
    );
  });
  ...
  socket.on("init-room", (userId) => {
    ...
    list_rooms = rooms.filter((elem) => {
      ...
      if (elem.user != undefined) return elem.user.includes(userId);
      else return false;
    });

    if (list_rooms == undefined) list_rooms = [];
  ...
  socket.on("init-chat-message", (idRoom) => {
    ...
    if (rooms.find((elem) => elem.id == idRoom)) {
      socketIo
        .to(idRoom)
        .emit("rooms", rooms.find((elem) => elem.id == idRoom).messages);
    }
  });
  ...
  socket.on("forward-message", async (data) => {
    ...
    let found_message = found_room.messages.find(
      (mess) => (mess.idMessage = data.idMessageToForward)
    );
  ...
  socket.on("remove-message", async (data) => {
    ...
    let found_message = found_room.messages.find(
      (mess) => mess.idMessage == data.idMessageToRemove
    );
    ...
    socketIo.to(data.idRoom).emit("chat-message-2", found_room.messages);
  });
  ...
  socket.on("delete-message", async (data) => {
    ...
    let found_message = found_room.messages.find(
      (mess) => mess.idMessage == data.idMessageToDelete
    );
